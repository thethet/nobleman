            ...
            <div class="panel-heading">
                <h3 class="panel-title">Attendance <span class="text-bold">List</span>
                    <div class="mig-wrap" style="float:right;padding-right:30px;">
                        {{ Form::open(array('url'=>'trainer/stdbylesson-filter/filterexport','method'=>'post')) }}   
                            <input type="hidden" name="getcourse" value="{{$getcourse}}" />
                ...
                <table class="display" id="sample_1">
                    ...
                    <?php                       
                        foreach($trainer_schedule as $key=> $ts_data){
                            $t_appointment = AppointmentEntry::where('date','=',$ts_data->date)->where('session','=',$ts_data->session)->where('course_id','=',$ts_data->course_id)->where('attend','!=','N/A')->where('trainer_id','=',$ts_data->trainer_id)->where('booking_status','=','book')->get();
